-- Migration 20: per-post aspect ratio (applies to all carousel images).
-- Run this in the Supabase SQL Editor. (Re-creates save_social_post to persist
-- aspect_ratio in addition to title.)

alter table social_posts add column if not exists aspect_ratio text not null default 'auto'
  check (aspect_ratio in ('auto','1:1','4:5','3:4','1.91:1'));
...
begin
  ...
  if (p_post->>'id') is null then
    insert into social_posts (title, caption, format, status, scheduled_at, posted_at, platforms, notes, aspect_ratio)
    values (
      nullif(p_post->>'title',''),
